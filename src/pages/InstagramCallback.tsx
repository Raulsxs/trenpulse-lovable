...
const InstagramCallback = () => {
  ...
  useEffect(() => {
    ...
    // Send code to parent window and process via edge function
    if (window.opener) {
      window.opener.postMessage({ type: "instagram-oauth-callback", code }, window.location.origin);
      setStatus("success");
  ...
  const processCallback = async (code: string) => {
    try {
      ...
      setStatus("success");
      setMessage(`Instagram conectado com sucesso! ${data?.connections?.[0]?.instagram_username ? `@${data.connections[0].instagram_username}` : ""}`);
      setTimeout(() => {
