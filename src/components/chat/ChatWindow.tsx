...
export default function ChatWindow() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // System-generated user messages from ActionCard buttons (Refazer, Adaptar) — user never typed these
      const SYSTEM_USER_COMMAND_PREFIXES = [
        "Refaça este conteúdo",
        "Adapte este conteúdo",
        "Regenere o último conteúdo",
      ];
      const data = (rawData || [])
        .filter((m: any) => {
          ...
          // Hide user messages generated by ActionCard buttons — they're not real user input
          if (m.role === "user" && SYSTEM_USER_COMMAND_PREFIXES.some(p => (m.content || "").trim().startsWith(p))) return false;
          return true;
        })
        .slice()
        .reverse();
  ...
  const handleSend = useCallback(async (text: string, extraParams?: Record<string, any>) => {
    ...
    setIsSending(true);
    const now = formatTime(new Date().toISOString());
    const isSilent = extraParams?.silent === true;
    if (!isSilent) {
    ...
    // Show loading indicator (skip when silent — ActionCard already shows feedback via toast/spinner)
    if (!isSilent) {
      setMessages((prev) => [...prev, { role: "assistant", content: "", isLoading: true }]);
    }
                ...
                <ChatMessage
                  ...
                  onRegenerate={() => {
                    ...
                    if (cid) {
                      ...
                      handleSend("Regenere o último conteúdo com um visual novo", { silent: true });
                    }
                  }}
                  onReject={() => {
