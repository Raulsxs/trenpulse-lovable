...
- REAPROVEITAR uma imagem que VOCÊ gerou como MÍDIA de um tweet card: se ele diz "usa essa imagem num tweet card", "faz um tweet card com a imagem que a gente gerou", chame gerar_tweet_card com usar_imagem_gerada=true (a imagem já existe — NUNCA peça pra anexar).
...
- Fotos anexadas (2+) + pedido de carrossel → use gerar_carrossel_editorial (elas viram o fundo). EXCEÇÃO: se o pedido for TWEET CARD, use gerar_tweet_card mesmo com fotos (elas viram mídia dentro do card).
...
Deno.serve(async (req) => {
  ...
  // Rede de segurança p/ as tools (ex.: link_para_post detectar carrossel/story mesmo se o LLM esquecer o campo).
  ctx.userText = incomingUserText;
